...
@Component({
  ...
  styles: [`
    ...
    .filter-tabs { display: flex; gap: var(--space-1); background: var(--color-bg-raised); border-radius: var(--radius-md); padding: 3px; }
    .filter-tab {
    ...
    .filter-tab.active { background: var(--color-bg-overlay); color: var(--color-text-primary); }
    .badge {
    ...
    .search-input {
      ...
      color: var(--color-text-primary); font-size: var(--text-sm); width: 200px;
    }
    ...
    .table tr:hover td { background: var(--color-bg-raised); }
    .th-actions, .td-actions { text-align: right; }

    .td-topic { max-width: 200px; font-family: var(--font-mono); font-size: var(--text-xs); }
    .td-window { white-space: nowrap; }
    .window-range { display: flex; align-items: center; gap: var(--space-2); color: var(--color-text-secondary); font-size: var(--text-xs); font-family: var(--font-mono); }
    .arrow { color: var(--color-text-muted); }
    .muted { color: var(--color-text-muted); font-size: var(--text-xs); }
    .mono { font-family: var(--font-mono); }

    .dim-tags { display: flex; gap: var(--space-1); flex-wrap: wrap; }
    .tag {
    ...
    .tag--dim { background: var(--color-bg-overlay); color: var(--color-text-secondary); }
    .tag--identity { background: rgba(79,142,247,0.12); color: var(--color-accent); }

    .status-badge {
      display: inline-flex; align-items: center; gap: var(--space-1);
      height: 22px; padding: 0 var(--space-2);
      border-radius: var(--radius-sm); font-size: var(--text-xs);
      font-weight: var(--font-weight-medium); text-transform: capitalize;
    }
    .status-badge--pending   { background: rgba(79,142,247,0.1); color: var(--color-accent); }
    .status-badge--confirmed { background: rgba(76,175,128,0.12); color: #4caf80; }
    .status-badge--missed    { background: rgba(232,168,56,0.1); color: var(--color-state-diverged); }
    .status-badge--cancelled { background: var(--color-bg-overlay); color: var(--color-text-muted); }

    .td-dims { min-width: 140px; }
    ...
    .empty-cta { color: var(--color-accent); text-decoration: none; }
    .empty-cta:hover { color: var(--color-accent-hover); }
...
export class ExpectedDivergenceListPage implements OnInit {
  ...
  setStatus(s: StatusFilter): void { this.activeStatus.set(s); }
  onSearchChange(): void { /* reactive via computed */ }
  ...
  statusIcon(status: string): import('@cav-align/ui').AppIconName {
    switch (status) {
      ...
      case 'missed':     return 'calendar-xmark';
