...
@Component({
          ...
          <div class="event-list">
            @for (evt of filtered(); track evt.id) {
              <div class="event-card" [class]="'event-card--' + getLabelClass(evt.label)">
                <!-- Label stripe -->
                <div class="label-stripe" [class]="'label-stripe--' + getLabelClass(evt.label)"></div>

                <div class="event-body">
                  <div class="event-top">
                    <div class="event-meta">
                      <span class="dim-badge dim-badge--{{ evt.dimension }}">{{ evt.dimension }}</span>
                      <span class="status-badge status-badge--{{ evt.status }}">{{ evt.status }}</span>
                      @if (evt.deviceId) {
                  ...
                  @if (evt.evidence) {
                    <div class="evidence-summary">
                      ...
                      @if (evt.evidence['affectedFieldPath']) {
                        <span class="evidence-field mono">{{ evt.evidence['affectedFieldPath'] }}</span>
                      }
                    </div>
                  }
                </div>
  ...
  styles: [`
    ...
    .page-body { flex: 1; padding: var(--space-6); overflow: auto; display: flex; flex-direction: column; gap: var(--space-4); }

    /* Legend */
    .legend { display: flex; gap: var(--space-6); flex-wrap: wrap; }
    ...
    .legend-dot--planned  { background: #4caf80; }
    .legend-dot--missed   { background: var(--color-state-diverged); }
    .legend-dot--unplanned { background: #e06c75; }

    /* Dim filter */
    .dim-tabs { display: flex; gap: var(--space-1); }
    .dim-tab {
    ...
    .dim-tab:hover { color: var(--color-text-primary); border-color: var(--color-border-default); }
    .dim-tab.active { background: var(--color-bg-raised); color: var(--color-text-primary); border-color: var(--color-border-default); }
    .badge {
      display: inline-flex; align-items: center; justify-content: center;
      min-width: 18px; height: 18px; padding: 0 4px;
      background: var(--color-bg-overlay); border-radius: 9px;
    ...
    .event-card--planned  { border-color: rgba(76,175,128,0.3); }
    .event-card--missed   { border-color: rgba(232,168,56,0.3); }
    .event-card--unplanned { border-color: rgba(224,108,117,0.2); }

    .label-stripe { width: 3px; flex-shrink: 0; }
    .label-stripe--planned  { background: #4caf80; }
    .label-stripe--missed   { background: var(--color-state-diverged); }
    .label-stripe--unplanned { background: #e06c75; }

    .event-body { flex: 1; padding: var(--space-4); display: flex; flex-direction: column; gap: var(--space-3); }

    .event-top { display: flex; align-items: flex-start; justify-content: space-between; gap: var(--space-4); }
    .event-meta { display: flex; align-items: center; gap: var(--space-2); flex-wrap: wrap; }

    .dim-badge {
      display: inline-flex; align-items: center;
      height: 20px; padding: 0 var(--space-2);
      border-radius: var(--radius-sm); font-size: var(--text-xs);
      font-weight: var(--font-weight-semibold); text-transform: uppercase; letter-spacing: 0.05em;
    }
    .dim-badge--shape   { background: rgba(79,142,247,0.12); color: var(--color-accent); }
    .dim-badge--cadence { background: rgba(107,90,173,0.15); color: #a78bfa; }
    .dim-badge--domain  { background: rgba(76,175,128,0.1); color: #4caf80; }
    ...
    .status-badge--confirmed { color: var(--color-state-diverged); background: rgba(232,168,56,0.08); }
    .status-badge--resolved  { color: var(--color-text-muted); }

    .device-tag {
      display: inline-flex; align-items: center;
      height: 20px; padding: 0 var(--space-2);
      background: rgba(79,142,247,0.08); border-radius: var(--radius-sm);
      font-size: var(--text-xs); font-family: var(--font-mono); color: var(--color-text-secondary);
    }

    .event-label {
      display: flex; align-items: center; gap: var(--space-1);
      font-size: var(--text-xs); font-weight: var(--font-weight-medium); white-space: nowrap;
    }
    .event-label--planned  { color: #4caf80; }
    .event-label--missed   { color: var(--color-state-diverged); }
    .event-label--unplanned { color: #e06c75; }

    .event-source {
      font-family: var(--font-mono); font-size: var(--text-xs);
      color: var(--color-text-secondary); max-width: 500px;
    }

    .event-times { display: flex; align-items: center; gap: var(--space-2); flex-wrap: wrap; }
    .time-item { display: flex; gap: var(--space-1); }
    .time-label { font-size: var(--text-xs); color: var(--color-text-muted); }
    .time-value { font-size: var(--text-xs); font-family: var(--font-mono); color: var(--color-text-secondary); }
    .time-sep { color: var(--color-text-muted); }

    .evidence-summary { display: flex; gap: var(--space-2); align-items: center; }
    .evidence-kind {
      display: inline-flex; align-items: center;
      height: 18px; padding: 0 var(--space-2);
      background: var(--color-bg-overlay); border-radius: var(--radius-sm);
      font-size: var(--text-xs); color: var(--color-text-secondary); text-transform: lowercase;
    }
    .evidence-field { font-size: var(--text-xs); color: var(--color-text-muted); }
    .mono { font-family: var(--font-mono); }
    .truncate { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }

    .empty-state {
      display: flex; flex-direction: column; align-items: center; gap: var(--space-4);
      padding: var(--space-16) var(--space-8); color: var(--color-text-muted); font-size: var(--text-sm);
    }
    .empty-state--error { color: var(--color-state-diverged); }
...
export class DivergenceLogPage implements OnInit {
  ...
  readonly labelConfig = [
    { label: 'confirmed_planned_change' as DivergenceEventLabel, display: 'Confirmed Planned Change', cssClass: 'planned' },
    { label: 'missed_planned_change'    as DivergenceEventLabel, display: 'Missed Planned Change',    cssClass: 'missed' },
    { label: 'unplanned_divergence'     as DivergenceEventLabel, display: 'Unplanned Divergence',     cssClass: 'unplanned' },
  ];
  ...
  readonly filtered = computed(() => {
    ...
    return dim === 'all'
      ? this.events()
      : this.events().filter((e) => e.dimension === dim);
  });
  ...
  load(): void {
    ...
    this.http
      .get<ApiResponse>(`${environment.apiUrl}/api/divergence`)
      .subscribe({
        next: (res) => {
  ...
  getLabelClass(label: DivergenceEventLabel): string {
    switch (label) {
      case 'confirmed_planned_change': return 'planned';
      case 'missed_planned_change':    return 'missed';
  ...
  getLabelIcon(label: DivergenceEventLabel): import('@cav-align/ui').AppIconName {
    switch (label) {
      case 'confirmed_planned_change': return 'calendar-check';
      case 'missed_planned_change':    return 'calendar-xmark';
      default:                         return 'bolt';
    }
  }
}
